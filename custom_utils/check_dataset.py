...
base_path = '../../../ssd_2/RecycleTrash/yolo/labels'

org = {
'train_list':['3rd', '4th', 'num', 'newpaperpack', 'newpet', 'pet_glass', '5th', '20201122', '20201203', '20201205', 'extra', 'joo_eun', 'newmix', 'kvl'],
'valid_list':['test'],
'test_list':['test']
}

type2 = {
'train_list':['3rd', '4th', 'num', 'newpaperpack', 'newpet', 'pet_glass', 'kvl'],
            ...
            for txt in txt_list:
                with open(txt) as f:
                    ...
                    for bbox in ann:
                        class_count[int(bbox[0])] += 1
                        all_count[int(bbox[0])] += 1
        print( ('%12s'+' : ['+'%6d'*7+'] -> sum : '+'%8d') % ( k, *class_count, class_count.sum()))
    print('-'*(12+6*7+5))
    print( ('%12s'+' : ['+'%6d'*7+'] -> sum : '+'%8d\n') % ( 'all', *all_count, all_count.sum()))
